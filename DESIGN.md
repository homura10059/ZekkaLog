...
カスタムカラーは使用しない。**SwiftUI / UIKit のセマンティックカラーのみ**で構成することで、  
ダークモード・高コントラストモードへの対応を自動化する。
...
| Role | SwiftUI | 用途 |
|---|---|---|
| Tint (Accent) | `.tint` / `Color(.tint)` | インタラクティブ要素（最小限の使用） |
...
- システムカラー（`.primary`, `.secondary`, `Color(.systemBackground)` 等）を優先する
